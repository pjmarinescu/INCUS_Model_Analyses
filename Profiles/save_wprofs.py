...
# Should update analysis to dot this manually..
# 20 km boundaries on G3
#DRC
#lat_bnds = [-5.318,-3.277] 
#lon_bnds = [24.238,25.563]
...
for g in np.arange(0,len(gridnums)): 
   ...
   path = '/nobackup/pmarines/PROD/'+casename+'/G3/out_30s/'
   ...
   #for f in np.arange(0,104,10): 
    ...
    #del(pres,temp,rv)
      ...
      savefile = '/nobackup/pmarines/Analysis_Plots/AIE/save_pickle/'+casename+'_G'+str(int(gridnum))+'_'+str(tc_thr)+str(w_thr)+str(max_depth)+'_'+str(npts)+'_'+cur_time+'_'+saveadd+'.p'
      ...
      # Save location of pickle file with w profiles that meet criteria specified below
      savefile = '/nobackup/pmarines/Analysis_Plots/AIE/save_pickle/'+casename+'_G'+str(int(gridnum))+'_'+svar_name+'_'+str(tc_thr)+str(w_thr)+str(max_depth)+'_'+str(npts)+'_'+cur_time+'_'+saveadd+'.p'

   # Save Dictionary Object as a Pickle File
   with open(savefile, 'wb') as handle:
      pickle.dump([save_w,save_tc,date_list], handle, protocol=pickle.HIGHEST_PROTOCOL)
